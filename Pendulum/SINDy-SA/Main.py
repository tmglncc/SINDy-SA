...
X = odeint(pend, X0, t, args=(b, c))

dd = DataDenoising(X, t, ["theta", "omega"])
dd.plot_sma([5, 10, 20])
	...
	for fourier_nfreq in fourier_nfreqs:
		for stlsq_alpha in stlsq_alphas:
			...
			optimizer = ps.STLSQ(
				...
				epsilon = 1.0e5,
				time = t,
				sa_times = np.array([10.0, 20.0, 30.0])
			)
...
for model_id, model in enumerate(model_set):
	...
	for init_cond_id, X0_test in enumerate(X0_tests):
		# Generate test data
		X_test = odeint(pend, X0_test, t, args=(b, c))
		...
		ax.plot(t, X_test[:,1], "k^", label = r"Data $y(t)$", alpha = 0.3, markersize = 5)
		ax.plot(t, simulation[:,0], "k-", label = r"Model $x(t)$", alpha = 1.0, linewidth = 1)
		ax.plot(t, simulation[:,1], "k--", label = r"Model $y(t)$", alpha = 1.0, linewidth = 1)
		...
		ax.set(xlabel = r"Time $t$", ylabel = r"$X(t)$",
			# title = "theta' = " + model.equations(precision = precision)[0] + "\n"
			# + "omega' = " + model.equations(precision = precision)[1] + "\n"
