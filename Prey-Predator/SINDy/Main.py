...
X0 = [4.0, 1.0]
X0_tests = [[4.0, 1.0]]
# X0_tests = [[4.0, 1.0], [3.5, 1.0], [3.0, 1.0], [1.0, 2.0], [1.5, 1.0]]
...
for model_id, model in enumerate(model_set):
	...
	for init_cond_id, X0_test in enumerate(X0_tests):
		# Generate test data
		X_test = odeint(lotka_volterra, X0_test, t, args=(alpha, beta, delta, gamma))
		...
		try:
			...
			ax.set(xlabel = r"Time $t$", ylabel = r"$X(t)$",
				# title = "x' = " + model.equations(precision = precision)[0] + "\n"
				# + "y' = " + model.equations(precision = precision)[1] + "\n"
