...
ax.set(xlabel = "Model complexity", ylabel = "Error", yscale = "log",
	...
	xticks = range(0, int(np.amax(ms.k))+3, 3)
)
ax.legend()
...
for i, j in zip(ms.k, ms.SSE):
	model_id += 1
	if model_id == 2:
		ax.annotate(str(model_id), xy = (i, j), xytext = (-23, 0), textcoords = 'offset points')
	elif model_id == 10:
		ax.annotate(str(model_id), xy = (i, j), xytext = (-30, -20), textcoords = 'offset points')
	else:
