...
ax.plot(np.delete(ms.k, ms.best_AICc_model), np.delete(ms.SSE, ms.best_AICc_model), "ko", alpha = 0.5, markersize = 10, label = "Other identified model")
ax.set(xlabel = "Model complexity", ylabel = "Error", yscale = "log",
	# title = optimizer_method + " - Pareto curve",
	xticks = range(0, int(np.amax(ms.k))+2, 1)
)
ax.legend()
